...
object TimerScheduler {
  /**
   * Java API: Factory method for [[TimerScheduler]]. `TimerScheduler` is
   * typically with `AbstractActorWithTimers` but in some case the single inheritance
   * slot is already taken, by for example `AbstractPersistentActor`, and then the
   * timers can be created and managed by composition instead. Then the timers must be
   * cancelled from `preRestart` and `postStop`.
   *
   * `TimerScheduler` is not thread-safe, i.e. it must only be used within
   * the actor that owns it.
   */
  def createTimerScheduler(context: AbstractActor.ActorContext): TimerScheduler =
    new TimerSchedulerImpl(context)
}
